...
public class ExoVideoView extends ResizingTextureView implements VideoViewApi, AudioCapabilitiesReceiver.Listener {
    ...
    @Override
    public void setVideoUri(@Nullable OkHttpClient okHttpClient, @Nullable Uri uri) {
        RenderBuilder builder = uri == null ? null : getRendererBuilder(okHttpClient, MediaSourceUtil.getType(uri), uri);
     ...
     */
    protected RenderBuilder getRendererBuilder(@NonNull OkHttpClient okHttpClient, @NonNull MediaSourceType renderType, @NonNull Uri uri) {
        switch (renderType) {
            case HLS:
                return new HlsRenderBuilder(getContext().getApplicationContext(), okHttpClient, getUserAgent(), uri.toString());
            case DASH:
                return new DashRenderBuilder(getContext().getApplicationContext(), okHttpClient, getUserAgent(), uri.toString());
            case SMOOTH_STREAM:
                return new SmoothStreamRenderBuilder(getContext().getApplicationContext(), okHttpClient, getUserAgent(), uri.toString());
            default:
                return new RenderBuilder(getContext().getApplicationContext(), okHttpClient, getUserAgent(), uri.toString());
        }
    }
