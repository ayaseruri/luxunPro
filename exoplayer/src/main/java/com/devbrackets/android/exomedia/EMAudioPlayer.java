...
public class EMAudioPlayer {
    ...
    public EMAudioPlayer(Context context, OkHttpClient okHttpClient) {
        this(context, okHttpClient, new DeviceUtil());
    }

    public EMAudioPlayer(Context context, OkHttpClient okHttpClient, DeviceUtil deviceUtil) {
        if (deviceUtil.supportsExoPlayer(context)) {
            mediaPlayerImpl = new ExoMediaPlayer(context, okHttpClient);
        } else {
